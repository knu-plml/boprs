...
func AccessControl(stub shim.ChaincodeStubInterface, function string, args []string) (UserType, error) {
    ...
    switch function {
        ...
        //args[0] = reportKey
        case "QueryReport", "QueryFullReport" :
            paperKey, err = CreatePaperKeyWithContractKey(stub, args[0])
            if err != nil {
                return userType, errors.New("AccessControl" + " : " + err.Error())
            }
...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
    ...
    if err != nil {
        return shim.Error("report : " + err.Error())
    }
    ...
    switch function {
        ...
        case "DeleteReportWithPaperKey" :
			err = s.DeleteReportWithPaperKey(stub, args)
        ...
        case "QueryFullReport" :
            result, err = s.QueryFullReport(stub, args)
...
func (s *SmartContract) AddReport(stub shim.ChaincodeStubInterface, args []string) error {
    ...
    _, err = InvokeChaincode(stub, "paper", "UpdateStatus", []string { paperKey, args[3] })
    if err != nil {
        return errors.New(function + " : " + err.Error())
    }
...
//args = [ paperKey ]
func (s *SmartContract) DeleteReportWithPaperKey(stub shim.ChaincodeStubInterface, args []string) error {
    ...
    reportIter, err = stub.GetStateByPartialCompositeKey("CR", splitPaperKey)
    if err != nil {
        return errors.New(function + " : " + err.Error())
    }
    ...
    for reportIter.HasNext() {
        ...
        err = stub.DelState(reportKV.Key)
        if err != nil {
            return errors.New(function + " : " + err.Error())
        }
    }
...
//args = [ reportKey ]
func (s *SmartContract) QueryFullReport(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
    var reportByte []byte
    var commentListByte []byte
    ...
    commentListByte, err = InvokeChaincode(stub, "comment", "QueryCommentWithContractKey", args)
    if err != nil {
        return nil, errors.New(function + " : " + err.Error())
    }
    if len(commentListByte) == 0 {
        return nil, errors.New(function + " : " + "There are no report for " + args[0] + " contract.")
    }

    buff.Write(reportByte[:len(reportByte)-1])
    buff.WriteString(", \"CommentList\":")
    buff.Write(commentListByte)
    buff.WriteString("}")
