...
async function main() {
    ...
    const user = argv[2];
    const reviewer = argv[3];
    const organization = argv[4];
    const paperKey = argv[5];

    const RPID = "".concat('\u0000', 'RP', '\u0000', reviewer, '\u0000', organization, '\u0000', paperKey, '\u0000');
    console.log(RPID);
    ...
    try {
        ...
        const result = await contract.evaluateTransaction('GetPRKey', RPID);
        console.log(`Transaction has been evaluated, result is: ${result.toString().replace(/\\u0000/g, '_')}`);
