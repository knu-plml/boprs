...
exports.main = async function main(user, paperKey, broker) {
    ...
    try {
        ...
        await contract.submitTransaction('StopRecruiting', paperID);
        console.log(`Transaction has been submitted. Stopped recruiting candidates for ${paperID} paper.`);

        const candidateList = await contract.evaluateTransaction('QueryCandidateIDWithPaperKey', paperID);
        console.log(`Transaction has been evaluated`);
        console.log(candidateList.toString());

        // Disconnect from the gateway.
        await gateway.disconnect();

        const message = '{\"Organization\":\"' + organization + '\",\"PaperID\":\"' + paperKey + '\",\"CandidateList\":' + candidateList.toString() + '}';
        ...
        if (queuedSuccess) {
            console.log(`queued ${paperID} candidate ${candidateList}.`);
        } else {
